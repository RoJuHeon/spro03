...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog"
	...
	$(document).ready(function() {
		...
		function showList(page) {
			...
			replyService.getList({bno : bnoValue, page : page || 1},//html를 만들어 마지막에 뿌리기
			function(replyCnt, list) {
				...
				console.log(list);
					...
					if(page==-1){//댓글이 1개 추가되면 마지막페이지를 다시 호출
						pageNum = Math.ceil(replyCnt/10.0);//마지막페이지도 보이게할려고
						showList(pageNum);
						return;
					}
					...
					for (var i = 0, len = list.length || 0; i < len; i++) {
						str += "<li class='left clearfix' data-rno='"+list[i].rno+"'>";//해당 번호 가져오기
	...
	modalRegisterBtn.on("click", function(e){
		...
		replyService.add(reply, function(result){
			...
			modal.find("input").val("");
			modal.modal("hide");
			showList(-1);//새 댓글 추가시 마지막페이지로 호출
		});
	});
...
replyPageFooter.on("click","li a", function(e){
	...
	showList(pageNum);
						...
						//	replyService.add(//함수호출, 2개의 매개변수 줌
						//			{reply:"JS TEST", replyer:"tester", bno:bnoValue},//데이터 주고
						//			function(result){//매개변수로 데이터 줌
						//				alert("RESULT : " + result);
						//			}
						//	);
						...
						//		replyService.remove(21, function(count){//7은 댓글이 있는 거 넣어주면됨
						//			console.log(count);
						//				if(count==="success"){
						//					alert("REMOVE 성공헸음.");
						//				}
