...
$('button').on("click", function(e){//버튼 클릭하면 이벤트 발생
	e.preventDefault();
	...
	if(operation === "remove"){
		...
		var pageNumTag = $("input[name='pageNum']").clone();//clone - 복제해
		var amountTag = $("input[name='amount']").clone();
		var typeTag = $("input[name='type']").clone();
		var keywordTag = $("input[name='keyword']").clone();
		formObj.empty();
		formObj.append(pageNumTag);
		formObj.append(amountTag);//append를 통해 주입
		formObj.append(typeTag);
		formObj.append(keywordTag);
//		self.location = "/board/list";//위에서 수정해서 써줬기 때문에 이 코드는 필요가 없음
	}
	formObj.submit();
	});
});
